...
func (b *OrganizationBudget) RecordUsage(ctx context.Context, record *UsageRecord) error {
	...
	for _, period := range []BudgetPeriod{BudgetPeriodDaily, BudgetPeriodWeekly, BudgetPeriodMonthly} {
		...
		// Update model usage
		if modelUsage, exists := usage.ModelUsage[record.ModelName]; exists {
			...
			modelUsage.CostUSD += record.CostUSD
		} else {
			usage.ModelUsage[record.ModelName] = &ModelUsage{
		...
		if providerUsage, exists := usage.ProviderUsage[record.ProviderID]; exists {
			...
			providerUsage.CostUSD += record.CostUSD
		} else {
			usage.ProviderUsage[record.ProviderID] = &ProviderUsage{
